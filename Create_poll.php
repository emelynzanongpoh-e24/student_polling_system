...
if(!isset($_SESSION['role'])){
    $r = mysqli_fetch_assoc(mysqli_query($conn, "SELECT role FROM users WHERE id='".$_SESSION['user_id']."'"));
...
if(isset($_POST['create'])){
    ...
    if(!empty($title) && !empty($expiry) && count($options) >= 2){
        ...
        foreach($options as $opt){
            ...
            if(!empty($opt)){
                mysqli_query($conn,"INSERT INTO poll_options(poll_id,option_text) VALUES('$poll_id','$opt')");
            }
        }
...
body{
    ...
    background-image: url("/student_polling_system/assets/images/dur2.webp");
